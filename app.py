...
set_background("https://images.unsplash.com/photo-1587517416820-6d98d2f7e08e?auto=format&fit=crop&w=1920&q=80")
...
def apply_equalizer(data, fs, gains):
    ...
    return processed / np.max(np.abs(processed))  # Normalize to prevent clipping
...
if uploaded_file is not None:
    ...
    if file_size_mb > 100:
        ...
        st.audio(uploaded_file, format="audio/mp3" if uploaded_file.name.endswith(".mp3") else "audio/wav")
        ...
        ax.set_title("Processed Audio Waveform", fontsize=12)
        st.pyplot(fig)
